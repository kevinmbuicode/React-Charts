...
import { Chart as Chartjs, LineElement, PointElement, CategoryScale, LinearScale } from 'chart.js'; //helps create the new ber chart by bringing in the bar element
...
const LineChart = () => {
    ...
    var data = {
        labels: chart?.coins?.map(coinName => coinName.name),
        datasets: [{
            label: `${chart.coins.length} Coins available`,
            data: chart?.coins?.map(coinName => coinName.price),
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
